...
let $albumCover := '/apps/contents/audioSources/' || $doc//id($albumCoverSurfaceID)/mei:graphic/@target
                            
(: TODO: Prüfen, ob die Pfade relativ sind :)

(:let $albumCover := '/apps/contents/audioSources/' || $doc//mei:meiHead/mei:fileDesc/mei:sourceDesc/mei:source/mei:physDesc/mei:extent[@label='cover']/mei:fig/mei:graphic/string(@target):)
let $records := for $rec in $doc//mei:recording
                ...
                let $avFile := '/apps/contents/audioData/' || $rec/mei:avFile[2]/string(@target)
                return
                    '{
    					...
    					,"album": "' || replace($album, '"', '\\"') || '"
    					,"url": "' || $avFile || '"
    					,"live": false' || (if($albumCover = '/apps/contents/audioSources/')then()else(
