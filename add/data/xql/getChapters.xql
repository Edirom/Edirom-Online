...
let $pageId := if ($mode = 'pageMode') then
...
return
    ...
    then
        ...
        let $label := if ($chapter/tei:head) then
            ...
            ($chapter/text())
        let $label := if (string-length($label) > 30) then
        ...
        if ($elem/local-name() = 'milestone' and $elem[@unit = 'number'])
        then
            (
            let $numbers := $elem
            let $n := concat('No. ', $numbers/@n)
                order by $numbers/number(replace(@n, '\D', '')),
                    $numbers/@n
            return
                concat('{',
            ...
            if ($elem/local-name() = 'div' and $elem[@type = 'act'])
            then
                (
                let $act := $elem
                ...
                return
                    ...
                    return
                        if (matches($t, '\w')) then
                            ...
                            (), '')
                    let $label := if (string-length($label) > 40) then
