...
let $doc := if ($page eq '') then
    ...
    return
        transform:transform($doc, doc('../xslt/reduceToPage.xsl'), <parameters><param
                name="pb1"
                value="{$pb1}"/><param
                name="pb2"
                value="{$pb2}"/></parameters>)
    )
...
<param
    name="lang"
    value="{eutil:getLanguage($edition)}"/>,
<param
    name="docUri"
    value="{$uri}"/>,
<param
...
<param
    name="base"
    value="{concat($base, '/../xslt/')}"/>,
<param
    name="footnoteBackLink"
    value="true"/>,
<param
    name="graphicsPrefix"
    value="{$imagePrefix}"/>, (:TODO frm XSLT <param name="graphicsPrefix"/>:)
<param
    name="numberHeadings"
    value="'false'"/>,
<param
...
let $doc := if ($xslInstruction) then
    (transform:transform($doc, doc($xsl), <parameters>{$params}</parameters>))
else
    (transform:transform($doc, doc($xsl), <parameters>{$params}</parameters>))

let $doc := transform:transform($doc, doc('../xslt/edirom_idPrefix.xsl'), <parameters><param
        name="idPrefix"
