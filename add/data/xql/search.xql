    ...
    class="searchResult">{
        ...
        return
            ...
            then
                ...
                then
                    (<div
                        class="searchResultOverview">Die Suche ergab Treffer in <span
                            class="num">{count($search)}</span> Objekten:</div>)
                else
                    (<div
                        class="searchResultOverview">Hits in <span
                            class="num">{count($search)}</span> objects:</div>)
                )
            else
                (if ($lang = 'de')
                then
                    (<div
                        class="searchResultOverview">Die Suche ergab keine Treffer.</div>)
                else
                    (<div
            ...
            return
                <div
                    ...
                    <div
                        class="doc"><span
                            class="resultTitle"
                            onclick="loadLink('xmldb:exist://{$uri}{
                            ...
                            }</span></div>
                    {
                        ...
                        let $internalId := if (local-name($hit) eq 'annot') then
                            ($hit/@xml:id)
                        else
                            if ($match/@xml:id) then
                                ($match/@xml:id)
                            else
                                ()
                        return
                            <div
                                ...
                                style="{
                                    ...
                                    kwic:get-summary($match, ($match/exist:match)[1], <config
                                        width="100"
                                        link="loadLink('xmldb:exist://{$uri}{
                                                if ($internalId) then
                                                    (concat('#', $internalId))
                                                else
                                                    ()
                        ...
                        then
                            (<div
                                class="showMore"
                                onclick="$(this).parent().find('div').show(); $(this).hide();">{
                                    if ($lang = 'de') then
                                        ('Alle Treffer zeigen')
                                    else
                                        ('show all hits')
