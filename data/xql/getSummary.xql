...
return
    ...
    then(local:getSourceSummary($doc, concat(request:get-scheme(), '://', request:get-server-name(), ':', request:get-server-port(), '/digilib/Scaler/')))
    else if($type = 'text')
    then(local:getTextSummary($doc, concat(request:get-scheme(), '://', request:get-server-name(), ':', request:get-server-port(), '/digilib/Scaler/')))
    else()
