...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:mei="http://www.music-encoding.org/ns/mei"
    xmlns:eof="http://www.edirom.de/xslt/ediromOnlineFunctions" xmlns:edirom="http://www.edirom.de"
    exclude-result-prefixes="xs xd fo mei edirom" xml:space="default" version="2.0">
    ...
    <xsl:output encoding="UTF-8" media-type="text/xml" method="xml" omit-xml-declaration="no"
        indent="yes"/>
            ...
            <div class="partMeasureSurfaceTabel">
                <h3>
                    <xsl:value-of select="@n | @label"
                        ><!-- TODO: welches ist der richtige Wert? --></xsl:value-of>
                </h3>
                <xsl:for-each-group select=".//mei:measure"
                    group-by=".//id(if(contains(@facs, '#'))then(substring-after(@facs, '#'))else(./@facs))/parent::mei:surface/@xml:id">
                    <dl>
                        <dt>
                            <xsl:value-of select="current-group()[1]/@n|current-group()[last()]/@n"
                                separator="–"/>
                        </dt>
                        <dd>
    ...
    <xsl:template match="/">
        <html>
            <head>
                <title>Print Preview <xsl:value-of
                        select="//mei:source/mei:identifier | //mei:source/mei:titleStmt/mei:title[1]"
                        separator=", "/></title>
                <link media="screen"
                    href="{concat($ediromOnlineRoot,'/resources/css/printPreview/screen.css')}"
                    type="text/css" rel="stylesheet"/>
                <link media="print"
                    href="{concat($ediromOnlineRoot,'/resources/css/printPreview/print.css')}"
                    type="text/css" rel="stylesheet"/>
            </head>
            <body>
                <xsl:apply-templates/>
                ...
                <div>
                    ...
                    <xsl:for-each select="//mei:mdiv">
                        <div class="property">
                            ...
                            <span class="value">
                                <xsl:value-of select="count(.//mei:measure)"/>
                                <xsl:text> </xsl:text>
                                <xsl:value-of select="eof:getLabel('measures')"/>
                            </span>
                        </div>
                    </xsl:for-each>
                </div>
    ...
    <xsl:template match="mei:surface">
        <div class="facsimile" id="{@xml:id}">
            <div class="static-before">
                <span>
                    <xsl:value-of select="//mei:titleStmt/*[data(.) != '']" separator=", "/>
                </span>
                <span>
            ...
            <div class="print-region-body">
                ...
                <div>
                    <xsl:element name="h3" use-attribute-sets="h1">
                        <xsl:value-of select="eof:getLabel('pageDetails')"/>
                    </xsl:element>
                    <div class="property">
                        <span class="key">
                            <xsl:value-of select="eof:getLabel('page')"/>
                    ...
                    <div class="property">
                        <span class="key">
                            <xsl:value-of select="eof:getLabel('movement')"/>
                            <xsl:value-of select="$pListKeyDelim"/>
                        </span>
                        <span class="value">
                            <xsl:value-of select="@n"/>
                    ...
                    <div class="property">
                        <span class="key">
                            <xsl:value-of select="eof:getLabel('measure(s)')"/>
                            <xsl:value-of select="$pListKeyDelim"/>
                        </span>
                        <span class="value">
                            <xsl:value-of
                                select="eof:getBarRangeStatement($measuersBySurface/eof:facs[@id = current()/@xml:id]/mei:measure,'–')"
                            />
                        </span>
                    </div>
                </div>
